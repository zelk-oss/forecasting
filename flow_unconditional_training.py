...
n_embedding = 64  # Reduced from 256 for small dataset (600 time points)
wave_length = 0.07  
...
for split in ["train", "val"]:
    ...
    # UNCONDITIONAL: Use normalized velocity fields (residuals) as targets
    # Model learns to generate velocity from pure noise (no initial condition)
    residual_norm = (vals[1:] - vals[:-1] - res_mean) / res_std
    datasets[split] = residual_norm
...
model = get_net(
    n_input=1,  # 1 channel: only intermediate_state (UNCONDITIONAL - no initial condition)
    n_output=1,
...
for epoch in pbar_epoch:  # OPTIONAL: Use epoch variable for logging
    ...
    for batch in pbar_train:        
        ...
        # UNCONDITIONAL: Use full batch as target (no separate initial condition)
        data_target = batch
        ...
        # Neural network input = only intermediate state (UNCONDITIONAL)
    ...
    for batch in pbar_val: 
        batch = batch.to(device=device, dtype=dtype)
        # UNCONDITIONAL: Use full batch as target
        data_target = batch
