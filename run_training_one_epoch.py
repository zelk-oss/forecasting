...
train_in = (torch.as_tensor(ds_train.values[:-1], dtype=dtype) - in_mean) / in_std
train_res = (torch.as_tensor(ds_train.values[1:] - ds_train.values[:-1], dtype=dtype) - res_mean) / res_std
train_data = torch.cat((train_in, train_res), dim=1)

val_in = (torch.as_tensor(ds_val.values[:-1], dtype=dtype) - in_mean) / in_std
val_res = (torch.as_tensor(ds_val.values[1:] - ds_val.values[:-1], dtype=dtype) - res_mean) / res_std
